...
foreach ($noti as $value):
    if ($value->data_devolucao == Carbon::now()->subDay(1)->toDateString()):
        $ontem[]=$value;
    elseif ($value->data_devolucao==Carbon::now()->toDateString()):
        $hoje[]=$value;
        ...
        <nav class="cyan darken-1">
            <div class="nav-wrapper row">
                ...
                <ul class="right col s9 m3 nav-right-menu">
                    <li><a href="javascript:void(0)" data-activates="notification" class="chat-button show-on-large"><i class="material-icons">notifications_none</i><?php if (count($noti)>0) echo "<span class=\"badge\"><script> var a=".count($noti).";document.write(a);</script></span>"?></a></li>
                </ul>
            </div>
        </nav>
    </header>
    <aside id="notification" class="side-nav white">
        <div class="side-nav-wrapper">
            <div id="sidebar-chat-tab" class="col s12 sidebar-messages right-sidebar-panel">
                ...
                if (isset($ontem)):
                    ?>
                    <div class="col l12 notification-drop-title">Ontem</div>
                    <div class="chat-list">
                        <?php
                        foreach ($ontem as $value):
                            $aluno= $tb_acervo->select()->
                            from('sisco.tb_aluno,sisco.tb_turma')->
                            where('sisco.tb_aluno.tb_turma_idtb_turma','idtb_turma','=',false)->
                            e('idtb_aluno',$value->tb_aluno_idtb_aluno)->
