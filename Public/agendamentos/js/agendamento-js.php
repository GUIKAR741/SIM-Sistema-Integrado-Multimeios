...
//            if (cont_valida===5){alert('abc')}
            if (cont_valida===5){envia(idAgenda,data,hora,recur,turma,professor);}
        });
        let envia=function (id,data,horario,recurso,turmas,professor){$.ajax({
            url:"ajax/atualiza.php",
            type: "POST",
            async: true,
            dataType: "JSON",
            data:{action:"true",id:id,data_submit:data,horario:horario,recurso:recurso,turmas:turmas,professor:professor},
