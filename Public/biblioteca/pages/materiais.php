...
if (isset($procura) && $procura!=''):
    ...
    $livros=$tb_acervo->select()->from()->like(
        "tipo_acervo = 'materiais' and titulo LIKE '%$procura%' or autor LIKE '%$procura%' ".
    ...
    $livros=$tb_acervo->select()->from()->limite("$pagina,$itens")->where('tipo_acervo','materiais')->all();
    #quantidade de livros mostrados
    $livroRow=$tb_acervo->count();
    #total de livros
    $total=$tb_acervo->select()->from()->where('tipo_acervo','materiais')->count();
endif;
...
if (isset($_GET['pag'])) if((intval($_GET['pag']) < 0) || ((intval($_GET['pag'])>=$num_pag))):
    echo "<script> location='?p=materiais'</script>";
endif;
#cadastro de livro
if (isset($_POST['enviarCAD'])):
    ...
    $tb_acervo->tipo_acervo="materiais";
    #campos
    #se foi enviado a imagem
    if ($_FILES['arquivo']['error']==UPLOAD_ERR_OK):
        #copia a imagem para pasta do upload
        $upload->image($_FILES['arquivo'],date("d-m-Y-H-i-s"),'materiais/');
        #redimensiona imagem para 250x350
        WideImage::load('../uploads/materiais/'.$upload->getName())->resize(250,350,'fill')->saveToFile('../uploads/materiais/'.$upload->getName());
    ...
    $tb_acervo->save();
    echo '<script>window.location=\'?p=materiais&livro=cadastrado\'</script>';
endif;
#editar livros
if (isset($_POST['enviarEDIT'])):
    ...
    if ($_FILES['arquivo']['error']==UPLOAD_ERR_OK):
        ...
        if (file_exists("../uploads/materiais/$value->capa") && is_file("../uploads/materiais/$value->capa")):
            $upload->excluir($value->capa,'materiais');
        endif;
        #copia a nova imagem
        $upload->image($_FILES['arquivo'],date("d-m-Y-H-i-s"),'materiais/');
        #redimensiona imagem para 250x350
    ...
    echo '<script>window.location=\'?p=materiais&livro=atualizado\'</script>';
endif;
#deletar livro
if (isset($_GET['delete']) && $_GET['delete']==true):
    ...
    echo "<script>document.location='?p=materiais&livro=deletado'</script>";
endif;
#cadastra a locação
if (isset($_POST['locacao'])):
    ...
    if (intval($attLivro->disponiveis)>0 && $aluno>0):
        ...
        echo "<script>document.location='?p=materiais&livro=locado&idaluno=".strip_tags($_POST['alunoSelect'])."&disponivel=true'</script>";
    endif;
    echo "<script>document.location='?p=materiais&livro=locado&idaluno=".strip_tags($_POST['alunoSelect'])."&disponivel=false'</script>";
endif;
                ...
                <div class="card-content">
                    <div class="row no-m">
                        <form method="post">
                            <div class="input-field col s12 m1">
                                <select name="itens" id="itens" onchange="window.location='?p=materiais&itens='+ $('#itens').val()">
                    ...
                    <table class="responsive-table" style="">
                        ...
                        if (isset($_POST['env'])):
                            ...
                            echo "<script>window.location='?p=materiais';</script>";
                        endif;
                            ...
                            <tr>
                                ...
                                <td class="no-m center no-p-h">
                                    <a class="btn-floating btn waves-effect waves-light red" onclick="swal({
                                            ...
                                            function(isConfirm){
                                            if (isConfirm) {
                                            location.href='?p=materiais&delete=true&del=<?= $value->idtb_acervo?>';
                                            } else {
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="col m12 l6">
                                        ...
                                        <div>
                                            ...
                                            if (file_exists("../uploads/materiais/$value->capa") && is_file("../uploads/materiais/$value->capa")):
                                                echo "<img src=\"../uploads/materiais/$value->capa\" class=\"col s6\">";
                                            else:
                                    ...
                                    <div class="col m12 l6">
                                        <div>
                                            ...
                                            <div class="input-field">
                                                <p class="range-field no-m">
                                                    <input type="range" class="no-m" id="ano" name="ano" min="1975" value="<?= intval($value->ano_publicacao)?>" max="<?= date('Y')?>"/>
                                                </p>
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <div class="input-field">
                                                <p class="range-field no-m">
                                                    <input type="range" class="no-m" id="exemplares" name="exemplares" min="1" value="<?= intval($value->exemplares)?>" max="100" />
                                                </p>
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <div class="input-field">
                                                <p class="range-field no-m">
                                                    <input type="range" class="no-m" id="volume" name="volume" min="1" value="<?= intval($value->volume)?>" max="50" />
                                                </p>
                                            </div>
                                        </div>
                                        <div class="input-field">
                            ...
                            <ul class="pagination no-m center">
                                <?php if (($pag-1)<0): ?>
                                    ...
                                    <li class="waves-effect">
                                        <a href="?p=materiais&pag=<?= $pag-1?>"><i class="material-icons">chevron_left</i></a>
                                    </li>
                                    <?php
                                endif;
                                if ($pag>=4):
                                ...
                                for ($i=$pagatual;$i<($paglist);$i++):
                                    if ($i>=0 && $i<$num_pag):
                                        ...
                                        <li <?php if ($i == $pag): echo "class='active'"; else: echo "class=\"waves-effect\""; endif;?>>
                                            <a href="?p=materiais&pag=<?= $i?>"><b><?= $i+1?></b></a>
                                        </li>
                                        <?php
                                    endif;
                                endfor;
                                if ($pag<=$num_pag-5):
                                    ...
                                    <li class="waves-effect">
                                        <a href="?p=materiais&pag=<?= $num_pag-1?>"><?= $num_pag?></a>
                                    </li>
                                    <?php
                                endif;
                                if ($pag<$num_pag-1):?>
                                    <li class="waves-effect">
                                        <a href="?p=materiais&pag=<?= $pag+1?>"><i class="material-icons">chevron_right</i></a>
                                    </li>
                                    <?php
                                else:?>
                                    <li class="disabled">
