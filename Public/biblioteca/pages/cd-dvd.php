...
if (isset($_POST['enviarCAD'])):
    ...
    $tb_acervo->tipo_acervo="cd-dvd";
    #campos
    #se foi enviado a imagem
    if ($_FILES['arquivo']['error']==UPLOAD_ERR_OK):
        #copia a imagem para pasta do upload
        $upload->image($_FILES['arquivo'],date("d-m-Y-H-i-s"),'cd-dvd/');
        #redimensiona imagem para 250x350
        WideImage::load('../uploads/cd-dvd/'.$upload->getName())->resize(250,350,'fill')->saveToFile('../uploads/cd-dvd/'.$upload->getName());
    ...
    $tb_acervo->save();
    echo '<script>window.location=\'?p=cd-dvd&livro=cadastrado\'</script>';
endif;
#editar livros
if (isset($_POST['enviarEDIT'])):
    ...
    if ($_FILES['arquivo']['error']==UPLOAD_ERR_OK):
        ...
        if (file_exists("../uploads/cd-dvd/$value->capa") && is_file("../uploads/cd-dvd/$value->capa")):
            $upload->excluir($value->capa,'cd-dvd/');
        endif;
        #copia a nova imagem
        $upload->image($_FILES['arquivo'],date("d-m-Y-H-i-s"),'cd-dvd/');
        #redimensiona imagem para 250x350
    ...
    echo '<script>window.location=\'?p=cd-dvd&livro=atualizado\'</script>';
endif;
#deletar livro
if (isset($_GET['delete']) && $_GET['delete']==true):
    ...
    echo "<script>document.location='?p=cd-dvd&livro=deletado'</script>";
endif;
#cadastra a locação
if (isset($_POST['locacao'])):
    ...
    if (intval($attLivro->disponiveis)>0 && $aluno>0):
        ...
        echo "<script>document.location='?p=cd-dvd&livro=locado&idaluno=".strip_tags($_POST['alunoSelect'])."&disponivel=true'</script>";
    endif;
    echo "<script>document.location='?p=cd-dvd&livro=locado&idaluno=".strip_tags($_POST['alunoSelect'])."&disponivel=false'</script>";
endif;
                            ...
                            <tr>
                                ...
                                <td class="no-m center no-p-h">
                                    <a class="btn-floating btn waves-effect waves-light red" onclick="swal({
                                            ...
                                            function(isConfirm){
                                            if (isConfirm) {
                                            location.href='?p=cd-dvd&delete=true&del=<?= $value->idtb_acervo?>';
                                            } else {
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="col m12 l6">
                                        ...
                                        <div>
                                            ...
                                            if (file_exists("../uploads/cd-dvd/$value->capa") && is_file("../uploads/cd-dvd/$value->capa")):
                                                echo "<img src=\"../uploads/cd-dvd/$value->capa\" class=\"col s6\">";
                                            else:
