...
if (isset($procura) && $procura!=''):
    ...
    $livros=$tb_acervo->select()->from()->like(
        "tipo_acervo = 'circulo' and titulo LIKE '%$procura%' or autor LIKE '%$procura%' ".
    ...
    $livros=$tb_acervo->select()->from()->limite("$pagina,$itens")->where('tipo_acervo','circulo')->all();
    #quantidade de livros mostrados
    $livroRow=$tb_acervo->count();
    #total de livros
    $total=$tb_acervo->select()->from()->where('tipo_acervo','circulo')->count();
endif;
...
if (isset($_GET['pag'])) if((intval($_GET['pag']) < 0) || ((intval($_GET['pag'])>=$num_pag))):
    echo "<script> location='?p=circulo'</script>";
endif;
#cadastro de livro
if (isset($_POST['enviarCAD'])):
    ...
    $tb_acervo->tipo_acervo="circulo";
    #campos
    #se foi enviado a imagem
    if ($_FILES['arquivo']['error']==UPLOAD_ERR_OK):
        #copia a imagem para pasta do upload
        $upload->image($_FILES['arquivo'],date("d-m-Y-H-i-s"),'circulo/');
        #redimensiona imagem para 250x350
        WideImage::load('../uploads/circulo/'.$upload->getName())->resize(250,350,'fill')->saveToFile('../uploads/circulo/'.$upload->getName());
    ...
    $tb_acervo->save();
    echo '<script>window.location=\'?p=circulo&livro=cadastrado\'</script>';
endif;
#editar livros
if (isset($_POST['enviarEDIT'])):
    ...
    if ($_FILES['arquivo']['error']==UPLOAD_ERR_OK):
        ...
        if (file_exists("../uploads/circulo/$value->capa") && is_file("../uploads/circulo/$value->capa")):
            $upload->excluir($value->capa,'circulo');
        endif;
        #copia a nova imagem
        $upload->image($_FILES['arquivo'],date("d-m-Y-H-i-s"),'circulo/');
        #redimensiona imagem para 250x350
    ...
    echo '<script>window.location=\'?p=circulo&livro=atualizado\'</script>';
endif;
#deletar livro
if (isset($_GET['delete']) && $_GET['delete']==true):
    ...
    echo "<script>document.location='?p=circulo&livro=deletado'</script>";
endif;
#cadastra a locação
if (isset($_POST['locacao'])):
    ...
    if (intval($attLivro->disponiveis)>0 && $aluno>0):
        ...
        echo "<script>document.location='?p=circulo&livro=locado&idaluno=".strip_tags($_POST['alunoSelect'])."&disponivel=true'</script>";
    endif;
    echo "<script>document.location='?p=circulo&livro=locado&idaluno=".strip_tags($_POST['alunoSelect'])."&disponivel=false'</script>";
endif;
                ...
                <div class="card-content">
                    <div class="row no-m">
                        <form method="post">
                            <div class="input-field col s12 m1">
                                <select name="itens" id="itens" onchange="window.location='?p=circulo&itens='+ $('#itens').val()">
                    ...
                    <table class="responsive-table" style="">
                        ...
                        if (isset($_POST['env'])):
                            ...
                            echo "<script>window.location='?p=circulo';</script>";
                        endif;
                            ...
                            <tr>
                                ...
                                <td class="no-m center no-p-h">
                                    <a class="btn-floating btn waves-effect waves-light red" onclick="swal({
                                            ...
                                            function(isConfirm){
                                            if (isConfirm) {
                                            location.href='?p=circulo&delete=true&del=<?= $value->idtb_acervo?>';
                                            } else {
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="col m12 l6">
                                        ...
                                        <div>
                                            ...
                                            if (file_exists("../uploads/circulo/$value->capa") && is_file("../uploads/circulo/$value->capa")):
                                                echo "<img src=\"../uploads/circulo/$value->capa\" class=\"col s6\">";
                                            else:
                            ...
                            <ul class="pagination no-m center">
                                <?php if (($pag-1)<0): ?>
                                    ...
                                    <li class="waves-effect">
                                        <a href="?p=circulo&pag=<?= $pag-1?>"><i class="material-icons">chevron_left</i></a>
                                    </li>
                                    <?php
                                endif;
                                if ($pag>=4):
                                ...
                                for ($i=$pagatual;$i<($paglist);$i++):
                                    if ($i>=0 && $i<$num_pag):
                                        ...
                                        <li <?php if ($i == $pag): echo "class='active'"; else: echo "class=\"waves-effect\""; endif;?>>
                                            <a href="?p=circulo&pag=<?= $i?>"><b><?= $i+1?></b></a>
                                        </li>
                                        <?php
                                    endif;
                                endfor;
                                if ($pag<=$num_pag-5):
                                    ...
                                    <li class="waves-effect">
                                        <a href="?p=circulo&pag=<?= $num_pag-1?>"><?= $num_pag?></a>
                                    </li>
                                    <?php
                                endif;
                                if ($pag<$num_pag-1):?>
                                    <li class="waves-effect">
                                        <a href="?p=circulo&pag=<?= $pag+1?>"><i class="material-icons">chevron_right</i></a>
                                    </li>
                                    <?php
                                else:?>
                                    <li class="disabled">
